...
If `export_json_schema` has no `__main__` entry point, read the bottom of the file and invoke it the way `test_schema_export.py` does.
...
def test_ack_expected_snapshots_active_department_members_excluding_author(database, fx):
    with database.session() as db:
        ...
        db.flush()
        assert fx.housekeeper_a.id not in entry.ack_expected
...
def _to_out(db: Session, entries: list[LogEntry], viewer_user_id: str) -> list[LogEntryOut]:
    ...
    for uid, first, last, avatar in db.execute(
            select(UserAccount.id, UserAccount.first_name, UserAccount.last_name,
                   UserAccount.avatar_url)).all():
        names[uid] = f"{first} {last}"
        avatars[uid] = avatar
    dept_names = dict(db.execute(select(Department.id, Department.name)).all())
...
def test_a_non_image_upload_is_rejected(app, fx, login):
    ...
    base = f"/api/p/{fx.property_a.id}/log-entries"
    agent = login("agent@hvh.test")
    res = agent.post(base, data={"body": "bad", "photo": (io.BytesIO(b"not an image"), "x.png")},
                     content_type="multipart/form-data")
