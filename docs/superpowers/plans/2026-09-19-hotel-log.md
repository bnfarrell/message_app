...
```ts
      { label: 'Log', to: '/app/log', icon: 'inbox', needs: [] },
```

If a more fitting `IconName` than `inbox` exists in `web/src/components/NavIcon.tsx`, use it; if not, leave `inbox` rather than inventing a new icon in this task.
...
`server/tests/test_seed.py` asserts on the seeded row counts. Add an assertion that three log entries exist for the seeded property, one pinned and one with a non-empty `ack_expected`.
