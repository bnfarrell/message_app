...
from app.schemas.enums import Direction, DraftPromptStatus, Priority, WorkOrderEventType, WorkOrderStatus, WorkOrderType
...
def test_assign_reassigns_and_notifies_new_assignee(app, fx, database):
    with database.session() as db:
        ...
        wo_id = wo.id
    with database.session() as db:
        wo = work_orders.assign(db, fx.property_a.id, wo_id, fx.supervisor_a.id, user_id=fx.housekeeper_a.id)
        ...
        # Filter by type rather than position: "created" and "assigned" share a created_at under the
        # frozen clock, and their relative order between two rows with unrelated (random) primary keys
        # is not something this query's ORDER BY guarantees.
        assigned_ev = db.scalar(select(WorkOrderEvent).where(WorkOrderEvent.work_order_id == wo_id,
                                                             WorkOrderEvent.type == WorkOrderEventType.assigned))
        assert assigned_ev is not None
        assert assigned_ev.from_value == fx.engineer_a.id and assigned_ev.to_value == fx.housekeeper_a.id
...
def test_dismiss_prompt_transitions_and_is_noop_once_sent(app, fx, client, database, login):
    ...
    with database.session() as db:
        wo2 = work_orders.create(db, fx.property_a.id, fx.agent_a.id, CreateWorkOrder(
            ...
            department_id=fx.dept_engineering.id, source_conversation_id=cid))
        wo2_id = wo2.id
        work_orders.transition(db, fx.property_a.id, wo2_id, fx.engineer_a.id, WorkOrderStatus.in_progress)
        work_orders.transition(db, fx.property_a.id, wo2_id, fx.engineer_a.id, WorkOrderStatus.complete)
    ...
    with database.session() as db:
        ...
        assert dp2.status == DraftPromptStatus.sent  # unchanged: dismiss only acts on pending prompts
...
def test_invalid_transition_is_409_and_audited(app, fx, database):
    with database.session() as db:
        ...
        ev = db.scalars(select(WorkOrderEvent).where(WorkOrderEvent.work_order_id == wo.id)).all()
        assert [e.type.value for e in ev] == ["created"]  # no spurious status_changed event was written
