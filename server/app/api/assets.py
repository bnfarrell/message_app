...
def list_assets(property_id: str):
    with db_session() as db:
        return ok(assets.list(db, g.property_id, include_inactive=request.args.get("includeInactive") in ("1", "true")))
...
def create_asset(property_id: str):
    with db_session() as db:
        return ok(AssetOut.model_validate(assets.create(db, g.property_id, parse_body(AssetIn))), 201)
...
def update_asset(property_id: str, asset_id: str):
    with db_session() as db:
        return ok(AssetOut.model_validate(assets.update(db, g.property_id, asset_id, parse_body(AssetPatch))))
